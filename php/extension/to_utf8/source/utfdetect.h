...
#define MAX_DETECT_NUM 256

inline int get_utf8_encoding_range(unsigned char *p) {
    int r = 0;
    switch (p[0]) {
        case 0xc2:
        case 0xc3: r = 1; break;
...
/* r>0: utf8, r=0: not utf8, r<0: unknown */ inline int is_utf8_encoding_strict(char *p, char *end) {
    ...
    while (p < end) {
        if (p[0] < 0) {
...
#ifdef __CHECK_RANGE
                int r = get_utf8_encoding_range((unsigned char *)p);
            ...
            if (++detect > MAX_DETECT_NUM) break;
        }
        else p++;
    }
#ifdef __DEBUG
...
/* r>0: utf8, r=0: not utf8, r<0: unknown */ inline int is_utf8_encoding_ignore(char *p, char *end, int skip = 10) {
...
inline int is_utf8_strict(char *p, char *end) {
    return is_utf8_encoding_strict(p, end); }
inline int is_utf8_ignore(char *p, char *end, int skip) {
    return is_utf8_encoding_ignore(p, end, skip); }
